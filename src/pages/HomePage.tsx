    ...
    <>
      ...
      <Box component="section" aria-label="Hero" sx={{ bgcolor: 'primary.main', color: 'primary.contrastText', py: { xs: 6, sm: 10 } }}>
        <Container maxWidth="md" sx={{ textAlign: 'center' }}>
          ...
          <Button component="a" href="/calculators" variant="contained" size="large" endIcon={<ArrowForwardIcon />} sx={{ bgcolor: 'background.paper', color: 'primary.main', '&:hover': { bgcolor: 'grey.100' }, px: 4, py: 1.5 }}>
        ...
        <Grid container spacing={2}>
          {calculators.map((c) => (
            <Grid size={{ xs: 12, sm: 6 }} key={c.meta.slug}>
              <Card sx={{ height: '100%' }}>
                <CardActionArea component="a" href={`/calculators/${c.meta.slug}`} sx={{ height: '100%' }}>
                  <CardContent>
      ...
      <Container component="section" aria-label="Categories" maxWidth="md" sx={{ py: { xs: 5, sm: 8 } }}>
        ...
        <Stack direction="row" spacing={1} sx={{ justifyContent: 'center', flexWrap: 'wrap', gap: 1 }}>
          {Object.entries(CATEGORY_LABELS).map(([key, label]) => (
            <Button key={key} component="a" href="/calculators" variant="outlined" size="small">
              {label}
            </Button>
          ))}
        </Stack>
        ...
        <Box sx={{ textAlign: 'center', mt: 5 }}>
          <Button component="a" href="/calculators" variant="contained" size="large" endIcon={<ArrowForwardIcon />}>
