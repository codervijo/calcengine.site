...
const meta: CalculatorMeta = {
  ...
  category: 'encoding',
  icon: 'Code',
  tagline: 'Enter the original byte count to see the exact Base64-encoded size and overhead. Useful for JWT sizing, cookie limits, and inlined image planning.',
  ...
  howItWorksTitle: 'How Base64 Encoding Increases Size',
  howItWorksImage: '/images/calculators/base64-size-how-it-works.svg',
  howItWorks: '1. Enter the original size of your data in bytes. Use the <a href="/calculators/json-size-calculator">JSON Size Calculator</a> for JSON payloads, or check file properties for files.\n2. The calculator applies the Base64 formula: every 3 input bytes become 4 output characters.\n3. Padding is accounted for — if the byte count isn\'t divisible by 3, one or two \'=\' padding characters are appended.\n4. Encoded size (bytes) and the percentage increase are shown instantly.',
  ...
  faq: [
    {
      question: 'Why does Base64 always increase size by ~33%?',
      answer: 'Base64 encodes 3 bytes as 4 characters, creating a fixed 4:3 ratio. That\'s one extra character per 3 bytes — exactly 33.3% overhead, every time, regardless of the content.',
    },
    {
      question: 'Does Base64 compress data?',
      answer: 'No. Base64 is an encoding format, not a compression algorithm — it increases size. Apply GZIP or Brotli compression before Base64-encoding if size is a concern. Compressing after encoding is ineffective because Base64 output has high entropy.',
    },
    {
      question: 'What is the maximum safe Base64 size for a browser cookie?',
      answer: 'Most browsers allow 4,096 bytes per cookie. A 3,000-byte JWT encodes to ~4,000 bytes in Base64, leaving very little headroom. If your JWT is growing, audit the claims and remove any unnecessary fields.',
    },
    {
      question: 'When should I use a URL instead of Base64 for images?',
      answer: 'For images over ~10 KB, a URL reference is almost always better. Base64-inlined images block HTML parsing, inflate page weight, and cannot be cached separately by the browser. Reserve Base64 inlining for tiny icons or critical-path assets.',
    },
    {
      question: 'Does Base64url (used in JWTs) change the encoded size?',
