...
import { type CalculatorMeta, CATEGORY_LABELS } from '../calculators/registry/types';
    ...
    <Container component="article" maxWidth="md" sx={{ py: { xs: 3, sm: 5 } }}>
      ...
      <nav aria-label="Breadcrumb">
        <MuiBreadcrumbs separator={<NavigateNextIcon sx={{ fontSize: 14 }} />} sx={{ mb: 2 }}>
          <MuiLink href="/" underline="hover" color="inherit" sx={{ fontSize: '0.85rem' }}>Home</MuiLink>
          <MuiLink href="/calculators" underline="hover" color="inherit" sx={{ fontSize: '0.85rem' }}>Calculators</MuiLink>
      ...
      {related.length > 0 && (
        <aside aria-label="Related calculators">
          ...
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
            {related.map((r) => (
              <Card key={r.meta.slug} component="a" href={`/calculators/${r.meta.slug}`} sx={{ flex: 1, textDecoration: 'none', '&:hover': { boxShadow: 3 }, transition: 'box-shadow 0.2s' }}>
                <CardContent>
