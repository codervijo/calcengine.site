...
Then read the row for `calcengine.site`. Or pull from
...
- **Action:** project scaffolded via `portfolio new bootstrap`; first deploy
  pending. After deploy: verify in GSC as `sc-domain:calcengine.site` and submit
  the sitemap.
